    ...
    <v-row class="mb-4" style="height: 80vh">
      ...
      <v-col lg="8">
        <v-layout class="wrap" style="max-height: 80vh; overflow-y: scroll;">
          <v-flex v-ripple md3 v-for="(item, index) in mDataArray" @click="pushNewOrder(item)" :key="index">
            <v-card class="ma-2 d-flex flex-column">
              <v-img :src="item.image | imageUrl" aspect-ratio="1" height="150">
                <v-avatar v-if="item.qty > 0" color="primary" size="36" class="ma-1 elevation-2">
                  <span class="white--text title">{{ item.qty }}</span>
                </v-avatar>
              </v-img>
              <div class="text-truncate px-4">{{ item.name }}</div>
              <div class="text-truncate px-4">D{{ item.product_id }} / ฿{{ item.price }}</div>
            </v-card>
          </v-flex>
        </v-layout>
      </v-col>
      ...
      <v-col lg="4">
        <v-card class="pa-5" style="height: 100%;">
          ...
          <div class="d-flex flex-row justify-space-between">
            ...
            <div>
              <span>
                <b style="font-size: 18px;font-weight: 100;">{{ mTaxAmt | currency("฿") }} </b>
              </span>
            </div>
          </div>
          ...
          <div class="d-flex flex-row justify-space-between">
            ...
            <div>
              <span>
                <b style="font-size: 27px; color: red;">{{ mTotalPrice | currency("฿") }} </b>
              </span>
            </div>
          </div>

          <Payment @onResult="onPaymentResult" :totalNumber="mTotalPrice" :order_list="JSON.stringify(mOrderArray)" />
          ...
          <div
            ...
            style="height: 80%;"
          >
            <img src="@/assets/waiting_for_sale.png" style="height: 200px; width: 200px;" />
          </div>

          <div class="d-flex flex-column mt-5" style="max-height: 55vh; overflow-y: scroll;">
            <v-card v-ripple outlined class="ma-2" v-for="(item, index) in mOrderArray" :key="index">
              <v-list-item>
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle> {{ item.price | currency("฿") }} x {{ item.qty }} pcs. </v-list-item-subtitle>
                </v-list-item-content>

                <v-list-item-avatar>
                  <v-icon color="#0003" @click="removeOrder(item)" v-ripple class="ml-2">
                    delete
                  </v-icon>
                </v-list-item-avatar>
              </v-list-item>
        ...
        <v-dialog v-model="payment_dialog" max-width="290">
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn color="green darken-1" text @click="payment_dialog = false">
                Close
              </v-btn>
            </v-card-actions>
          </v-card>
