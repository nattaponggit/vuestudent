  ...
  <v-container>
    ...
    <v-row>
      <v-col lg="3" md="6" sm="12" cols="12">
        <StockCard title="TOTAL" avatar_ic="mdi-cart-outline" avatar_bg="#00a65a" subtitle="1,800" />
      </v-col>

      <v-col lg="3" md="6" sm="12" cols="12">
        <StockCard title="SOLD-OUT" avatar_ic="mdi-flask-empty-outline" avatar_bg="#f39c12" subtitle="12" />
      </v-col>

      <v-col lg="3" md="6" sm="12" cols="12">
        <StockCard title="RETURN" avatar_ic="mdi-keyboard-return" avatar_bg="#dd4b39" subtitle="2" />
      </v-col>

      <v-col lg="3" md="6" sm="12" cols="12">
        <StockCard title="DISCOUNT" avatar_ic="mdi-gift-outline" avatar_bg="#00c0ef" subtitle="101" />
      </v-col>
    </v-row>

    <v-card>
      <v-data-table :search="search" :headers="headers" :items="mDataArray" :items-per-page="7" class="elevation-1">
        <template v-slot:top>
        ...
        <template v-slot:item="{ item }">
          <tr :class="{ primary: item.product_id === selectedProductId }">
            ...
            <td @click="editItem(item)">{{ item.stock | number("0,0") }} pcs.</td>
            <td>
