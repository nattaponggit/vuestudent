  ...
  <v-container>
    <v-row>
        ...
        <v-card>
          ...
          >
            ...
            <template v-slot:item="{ item }">
              <tr :class="selectedTransactionId == item.transaction_id ? 'primary' : null">
      ...
      <v-col v-if="selectedOrderList.length > 0" :cols="4">
        <v-card>
          <v-data-table :headers="headersOfDetail" :items="selectedOrderList" class="elevation-1">
            <template v-slot:top>
              <v-toolbar flat color="white">
                <v-toolbar-title>
                  <span class="primary--text"> TransactionId : {{ selectedTransactionId }} </span>
                </v-toolbar-title>
                <v-spacer></v-spacer>
